...
def get_encoder():
    with open("../data/gpt-2/encoder.json", "r") as file:
        encoder = json.load(file)
    with open("../data/gpt-2/vocab.bpe", "r", encoding="utf-8") as file:
        bpe_data = file.read()
...
def text_generator(model_weight):
    batch_size = 1
    max_seq_len = -1
    temperature = 0.7
    top_k = 40
    ...
    model = load_weight(model, model_weight)
    model.to(device)
    model.eval()

    if max_seq_len == -1:
        max_seq_len = config.n_ctx // 2
    elif max_seq_len > config.n_ctx:
        raise ValueError(f"Can't get samples longer than window size: {config.n_ctx}")
    ...
    for _ in range(n_samples // batch_size):
        outputs = sample_sequence(
            ...
            start_token=encoder.encoder["<|endoftext|>"] if unconditional else None,
        ...
        for i in range(batch_size):
            generated += 1
            text = encoder.decode(outputs[i])
            print("=" * 40 + " SAMPLE " + str(generated) + " " + "=" * 40)
            print(text)


if __name__ == "__main__":
    model_file = "../data/gpt-2/gpt2-pytorch_model.bin"
    if os.path.exists(model_file):
        model_weight = torch.load(
            model_file, map_location="cpu" if not torch.cuda.is_available() else None
        )
        text_generator(model_weight)
    else:
