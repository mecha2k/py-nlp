...
def gradients(ys, xs, grad_ys=None, checkpoints="collection", **kwargs):
    ...
    if xs_intersect_checkpoints:
        debug_print(
            "Warning, some input nodes are also checkpoint nodes: %s", xs_intersect_checkpoints
        )
    ...
    d_checkpoints = {
        r: dr for r, dr in zip(checkpoints_disconnected.keys(), dv[: len(checkpoints_disconnected)])
    }
