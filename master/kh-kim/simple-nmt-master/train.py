...
def define_argparser(is_continue=False):
    ...
    p.add_argument(
        "--n_epochs", type=int, default=20, help="Number of epochs to train. Default=%(default)s"
    )
    p.add_argument(
        "--verbose",
        type=int,
    ...
    p.add_argument(
        "--hidden_size", type=int, default=768, help="Hidden size of LSTM. Default=%(default)s"
    )
    p.add_argument(
        "--n_layers", type=int, default=4, help="Number of layers in LSTM. Default=%(default)s"
    )
    p.add_argument(
        "--max_grad_norm",
        type=float,
...
def main(config, model_weight=None, opt_weight=None):
    ...
    # Pass tf-models to GPU device if it is necessary.
    if config.gpu_id >= 0:
