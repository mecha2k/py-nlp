...
def define_argparser():
    ...
    p.add_argument(
        "--gpu_id", type=int, default=-1, help="GPU ID to use. -1 for CPU. Default=%(default)s"
    )
    ...
    p.add_argument(
        "--beam_size", type=int, default=5, help="Beam size for beam search. Default=%(default)s"
    )
    p.add_argument(
        "--lang", type=str, default=None, help="Source language and target language. Example: enko"
    )
    p.add_argument(
        "--length_penalty",
        type=float,
...
if __name__ == "__main__":
    ...
    # Put tf-models to device if it is necessary.
    if config.gpu_id >= 0:
