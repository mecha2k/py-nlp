...
if __name__ == "__main__":
    ...
    for line in lines:
        ...
        test_pred_list.append(
            {"sum_sents_tokenized": sum_sents_text, "sum_sents_idxes": sum_sents_idx_list}
        )

    result_df = pd.merge(
        test_df, pd.DataFrame(test_pred_list), how="left", left_index=True, right_index=True
    )
    result_df["summary"] = result_df.apply(
        lambda row: "\n".join(list(np.array(row["article_original"])[row["sum_sents_idxes"]])),
        axis=1,
    )
    ...
    submit_df = pd.merge(
        submit_df, result_df.loc[:, ["id", "summary"]], how="left", left_on="id", right_on="id"
    )
