    ...
    if len(sents_splited) == 1:
        ...
        for i in range(len(sents_splited) - 1):
            ...
            if sents_splited[idx][-1] not in [".", "?"] and idx < len(sents_splited) - 1:
                sents_splited[idx] = (
                    sents_splited[idx] + " " + sents_splited[idx + 1]
                    if doc[len(sents_splited[0])] == " "
...
def create_json_files(df, data_type="train", target_summary_sent=None, path=""):
    ...
    for start_idx in tqdm(start_idx_list):
        ...
        file_name = (
            os.path.join(
                f"{path}/{data_type}_{target_summary_sent}"
                + f"/{data_type}.{start_idx_str}_{end_idx_str}.json"
            )
            if target_summary_sent is not None
            else os.path.join(
                f"{path}/{data_type}" + f"/{data_type}.{start_idx_str}_{end_idx_str}.json"
            )
        )
        ...
        for i, row in df.iloc[start_idx:end_idx].iterrows():
            # original_sents_list = []
            # for original_sent in row.text:
            #     if original_sent:
            #         print(original_sent[0]["sentence"])
            #         original_sents_list.append(preprocessing(original_sent[0]["sentence"]).split())

            original_sents_list = [
                preprocessing(original_sent[0]["sentence"])  # , korean_tokenizer
                for original_sent in row["text"]
                if original_sent
            ]
            ...
            if target_summary_sent is not None:
                if target_summary_sent == "ext":
                    summary_sents = row["extractive_sentence"]
                else:
                    summary_sents = korean_sent_spliter(row["abstractive"])
                summary_sents_list = [
                    preprocessing(summary_sents)
                    # .split()  # , korean_tokenizer
...
if __name__ == "__main__":
    ...
    if args.task == "df":  # and valid_df
        ...
        df["extractive_sents"] = df.apply(
            lambda row: list(np.array(row["article_original"])[row["extractive"]]), axis=1
        )
    ...
    # Make bert input file for train and valid from df file
        ...
        for data_type in ["train", "valid"]:
            ...
            create_json_files(
                df,
                data_type=data_type,
                target_summary_sent=args.target_summary_sent,
                path=JSON_DATA_DIR,
            )
            ...
            preprocess_cmd = (
                f"python preprocess.py -mode format_to_bert "
                f"-dataset {data_type} -raw_path {json_data_dir} "
                f"-save_path {bert_data_dir} -log_file {LOG_PREPO_FILE} "
