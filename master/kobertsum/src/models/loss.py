...
def abs_loss(generator, symbols, vocab_size, device, train=True, label_smoothing=0.0):
    compute = NMTLossCompute(
        generator, symbols, vocab_size, label_smoothing=label_smoothing if train else 0.0
    )
    compute.to(device)
    return compute
...
def shards(state, shard_size, eval_only=False):
    ...
    if eval_only:
        ...
        keys, values = zip(
            *((k, [v_chunk for v_chunk in v_split]) for k, (_, v_split) in non_none.items())
        )
        ...
        for k, (v, v_split) in non_none.items():
            if isinstance(v, torch.Tensor) and state[k].requires_grad:
                variables.extend(
                    zip(torch.split(state[k], shard_size), [v_chunk.grad for v_chunk in v_split])
                )
