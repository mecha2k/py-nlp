...
class FSDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        linecache_index -= total_lines_in_other_files  # remove lines counted from other files
...
class SentencesProcessor:
    ...
    def get_features_process(
        ...
        if create_sent_rep_token_ids:
            ...
            if create_sent_lengths:
                ...
                if len(sent_rep_ids) < 2:
                    ...
                    sent_lengths = [
                        sent_rep_ids[i] - sent_rep_ids[i - 1] for i in range(1, len(sent_rep_ids))
                    ]
        ...
        if ex_index < 5 and self.verbose:
            ...
            if sent_lengths is not None:
                logger.info("sent_lengths: %s", " ".join([str(x) for x in sent_lengths]))
            if create_attention_mask:
    ...
    def get_features(
        ...
        if not return_type:
            ...
            all_labels = torch.tensor(pad([f.labels for f in features], 0), dtype=torch.long)
